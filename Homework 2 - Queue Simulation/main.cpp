...
int main()
{
	...
	cout << "Average time between clients arrival: " << betweenArrival << endl
		 << endl;
	...
	for (clock = 1; clock <= time; clock++)
	{
		...
		if (p / 100.0 > exp(-lambda))
		{
			...
			cout << "Client number " << ++clientsArrived << " arrived at time step " << clock << " with " << client.getNumOfPackets() << " packets." << endl;
